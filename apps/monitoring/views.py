...
def patient_vital_reading_create(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        messages.success(request, lang_text("Показатели сохранены.", "Көрсеткіштер сақталды."))
        return redirect("patient-vital-reading-list")
    return render(
        request,
        "monitoring/vital_form.html",
        {"form": form, "title": lang_text("Новые показатели", "Жаңа көрсеткіштер")},
    )
...
def staff_patient_vital_create(request, patient_pk: int):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        messages.success(request, lang_text("Показатели пациента сохранены.", "Пациент көрсеткіштері сақталды."))
        return redirect("staff-patient-vital-list", patient_pk=patient.pk)
    return render(
        request,
        "monitoring/vital_form.html",
        {"form": form, "title": lang_text(f"Новые показатели: {patient}", f"Жаңа көрсеткіштер: {patient}")},
    )
